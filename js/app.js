...
// 担当者モーダルは #modal-root、画面本体は #app と置き場所が別なので、
// モーダルが開いていても後ろは描き直してよい。
// 止めていたころは、モーダルを開けたまま待っていると後ろが古いままで、
// 現場から「固まった」に見えていた。
onCacheChange(() => {
  const route = parseRoute();
  if (route.kind === 'tab' && !document.querySelector('.fullpage')) render();
  if (staffModalOpen) openStaffModal(!!state.staff);   // 一覧を最新にして開き直す
});
